...
| 14/04/2026 | 1.1 | Actualización al estado real de implementación: corrección de stack tecnológico, eliminación de funcionalidades no implementadas (PDF, acceso anónimo, tabla AUDIT_LOGS, Audit Service, ROLE_SYSTEM_ADMIN), corrección del flujo de recuperación de contraseña (OTP), actualización del modelo de sincronización con Google Calendar | Equipo TeachingPlanner |
...
6. **Transparencia**: Consulta de horarios por personal académico autenticado (acceso público sin autenticación previsto en v2.0)
...
- **Vite 6+**: Build tool para aplicaciones web modernas
- **MariaDB 11+**: Sistema de gestión de bases de datos
...
- **Vista de Horarios**: Consulta de horarios por titulación y semestre (requiere autenticación en v1.0)
- **Portal del Profesor**: Panel limitado para profesores con funciones específicas

#### B) Interfaz de Hardware
- **Servidor de Aplicaciones**: Desplegado en infraestructura de la Universidad (Azure VM)
- **Servidor de Base de Datos**: MariaDB en servidor dedicado (dos bases de datos: `management_db` y `planner_db`)
...
│ usuarios/auth     │   │ calendarios,      │
│                   │   │ eventos, grupos   │
└───────────────────┘   └───────────────────┘
```

**Servicios:**
1. **Gateway Service**: Punto de entrada único, enrutamiento y autenticación JWT
2. **Auth Service**: Autenticación, activación de cuentas, recuperación de contraseñas, OAuth con Google
3. **User Service**: Gestión de usuarios y perfiles
4. **Planner Service**: Lógica de negocio principal (calendarios, grupos, eventos, solicitudes, sincronización Google Calendar)
...
### 2.2.7 Visualización de Horarios (RF-VIEW)
Consulta de horarios del calendario académico:
- Vista de horarios por curso y semestre
- Filtrado por titulación, curso, asignatura
- Vista de calendario con eventos periódicos y puntuales
- Visualización de cancelaciones

> **Nota v1.0:** El acceso a la vista de horarios requiere autenticación en la versión actual. El acceso público sin autenticación está previsto como mejora en versiones futuras.

### 2.2.8 Auditoría y Trazabilidad (RF-AUDIT)
Registro de actividad mediante metadatos en entidades:
- Campos de auditoría automáticos en todas las entidades: `createdBy`, `createdAt`, `updatedBy`, `updatedAt`
- Identificación del responsable de cada creación y modificación
- Registro del timestamp exacto de cada operación

> **Nota v1.0:** La interfaz de consulta de auditoría (página de Logs) y la exportación CSV de logs están previstas en versiones futuras. En v1.0 la auditoría es accesible directamente en los metadatos de cada entidad.

### 2.2.9 Exportación de Datos (RF-EXPORT)
Generación y descarga de datos:
- **Exportación ZIP de calendario**: Descarga completa de un calendario con tres ficheros: `ubicaciones.txt` (aulas en formato `CÓDIGO:URL_GIS`), `asignaturas.txt` (datos académicos de asignaturas con grupos por tipo e idioma) y `calendario.txt` (eventos programados de tipo NORMAL)
- **Exportación CSV formato Google Calendar**: Generación de CSV compatible con Google Calendar para importación directa
- **Importación de usuarios desde Excel**: Carga masiva de usuarios desde fichero XLSX
...
> **Nota v1.0:** El acceso de usuarios anónimos no está implementado en la versión actual. Todas las funcionalidades del sistema requieren autenticación. El acceso público a la consulta de horarios está previsto en versiones futuras del sistema.

**Funciones Previstas (v2.0+):**
- Consultar horarios públicos sin registro
...
- Consultar horarios por titulación, curso y semestre
...
- Conectar cuenta con Google y habilitar sincronización de calendarios de aulas
...
- CRUD completo de todos los elementos (titulaciones, asignaturas, grupos, aulas, calendarios, eventos)
- Aprobación/rechazo de solicitudes de cambio del profesorado
- Gestión de usuarios y roles (crear, editar, importar desde Excel)
- Configuración y duplicación de calendarios académicos
- Resolución de conflictos de horario (detectados automáticamente)
- Sincronización de calendarios académicos con Google Calendar (por aula)
- Exportación de calendarios en formato ZIP
...
- Backend: Node.js 20+ con TypeScript 5+, Express 5+
- Frontend: React 19+ con TypeScript, Vite 6+
...
- **Servidor de BD**: MariaDB 11+ con almacenamiento suficiente
...
> **Nota v1.0:** El acceso de estudiantes sin cuenta propia está previsto para versiones futuras. En v1.0 la consulta de horarios requiere autenticación.
...
| N-05 | STK-04 | Consulta de horarios (requiere autenticación en v1.0; acceso público previsto en v2.0) | ALTA | RF-VIEW-01 |
...
- **Mejora en comunicación:** Sincronización automática con Google Calendar
- **Accesibilidad:** Consulta de horarios 24/7 con autenticación (acceso público previsto en v2.0)
...
- **Profesores (80-120 usuarios):** Consulta de planificación, solicitudes de cambio, sincronización Google Calendar
- **Usuarios autenticados:** Consulta de horarios por titulación y semestre

## Alcance Técnico

- **Arquitectura:** Microservicios con Docker (4 servicios: Gateway, Auth, User, Planner)
- **Frontend:** React 19+ con TypeScript y Vite 6+
- **Backend:** Node.js 20+ con Express 5+ y TypeORM 0.3+
- **Base de Datos:** MariaDB 11+ (dos instancias: `management_db` y `planner_db`)
- **Integraciones:** Google Calendar API v3, Email SMTP

## Requisitos Destacados

### Requisitos Críticos
1. **Detección automática de conflictos** de horario antes de crear eventos
2. **Vista de horarios** por titulación, curso y semestre
3. **Seguridad y cumplimiento RGPD** con encriptación y auditoría de entidades
...
- Sincronización con Google Calendar por aula
- Exportación ZIP de calendarios e importación desde ficheros de texto
- Auditoría de entidades con metadatos (responsable y timestamp)
